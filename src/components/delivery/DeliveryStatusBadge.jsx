...
export const DeliveryStatusBadge = ({ status }) => {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 9px', borderRadius: 99, background: cfg.bg, color: cfg.color, fontSize: 11.5, fontWeight: 600, fontFamily: 'Outfit, sans-serif', whiteSpace: 'nowrap' }}>
...
export const PricingTierBadge = ({ tier }) => {
  ...
  return (
    <span style={{ fontSize: 11, background: `${color}18`, color, padding: '2px 9px', borderRadius: 99, fontWeight: 700, fontFamily: 'Outfit, sans-serif', border: `1px solid ${color}30` }}>
...
// ── InvoicePreview ────────────────────────────────────────────────────────────
// Inline preview card with download button
export const InvoicePreview = ({ sale }) => {
...
export const CreditNoteButton = ({ ret }) => (
  <button
    ...
    style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 12px', borderRadius: 7, border: '1px solid #bbf7d0', background: '#f0fdf4', color: '#16a34a', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}
  >
...
export const CreditStatusBadge = ({ creditLimit, currentBalance }) => {
  if (!creditLimit || creditLimit === 0) {
    return <span style={{ fontSize: 11, background: 'var(--content-bg)', color: 'var(--text-muted)', padding: '2px 8px', borderRadius: 99, fontWeight: 600 }}>Cash only</span>
  }
